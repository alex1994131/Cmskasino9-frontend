...
class Child extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    handleSubmit = (e)=>{
      ...
      if(!this.state.update){
        var row = {
          ...
          // priceWin : this.state.priceWin,
          // gainPrice :this.state.gainPrice
        }
    ...
    rowEdit(row){
      this.me.setState({gainPrice : row.gainPrice,name : row.name,modal : true,image : row.image,bidPrice : row.bidPrice,priceWin : row.priceWin,rowid : row._id,update : true,ordernum : row.order,isChecked : row.status,bool : row.bool});
    }
      ...
      <div  id="admindata_table"  className={`data-list list-view`}>
          ...
          <Form onSubmit={this.handleSubmit}  action={history.location.pathname} >
            ...
            <ModalBody className="modal-dialog-centered  mt-1">
              <Row>
                ...
                {/* <Col md="12">
                  <FormGroup className="form-label-group position-relative has-icon-left">
                    <Input type="text"  placeholder="bidPrice" value={this.state.bidPrice} 
                    onChange={e => this.setState({ bidPrice: e.target.value })} required />
                    <div className="form-control-position" >
                      <Link size={15} />
                    </div>
                ...
                <Col md="12">
                  <FormGroup className="form-label-group position-relative has-icon-left">
                    <Input type="text" placeholder="priceWin" value={this.state.priceWin}
                      onChange={e => this.setState({ priceWin: e.target.value })} required />
                    <div className="form-control-position" >
                      <Award size={15} />
                    </div>
                    <Label>priceWin</Label>
                  </FormGroup>
                </Col>
                <Col md="12">
                  <FormGroup className="form-label-group position-relative has-icon-left">
                    <Input type="text" placeholder="gainPrice" value={this.state.gainPrice}
                      onChange={e => this.setState({ gainPrice: e.target.value })} required />
                    <div className="form-control-position" >
                      <Award size={15} />
                    </div>
                    <Label>gainPrice</Label>
                  </FormGroup>
                </Col> */}
                <Col md="12">
                  <FormGroup className="form-label-group position-relative has-icon-left">
        ...
        {/* <Modal isOpen={this.state.imgmodal} toggle={this.ImgtoggleModal} className="modal-dialog-centered " >
          ...
          <ModalBody className="modal-dialog-centered  mt-1 d-block">
            <Row>
              <Col md="12">
                <CustomInput type="file" label="choose" id="gameslistmatka" accept="image/*" onChange={this.onFileChange} name="imgs" />
              </Col>
              <Col md="12">
                {
                  this.state.imageSrc && this.state.imageSrc.length > 0 ?  
                  <img alt="" src={this.state.imageSrc} />  :null
                }
              </Col>
            </Row>
          </ModalBody>
          <ModalFooter>
            <Button color="primary" type="button" onClick={()=>this.fileupload}>update</Button>
          </ModalFooter>
        </Modal> */}
