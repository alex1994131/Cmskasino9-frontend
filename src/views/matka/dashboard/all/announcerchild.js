...
import {selectedStyle,pagenation_set,games_id_key} from "../../../../configs/providerconfig"
...
class Child extends Component {
  ...
  state = {
    ...
    columns: [
      ...
      {
        name: "Single Pana",
        selector: "Single Pana",
        sortable: false,
        minWidth: "110px",
        cell : row =>(
            <div>
            {
                row[games_id_key["single pana"]]
            }
            </div>
        )
      },
      {
        name: "Double Pana",
        selector: "Double Pana",
        sortable: false,
        minWidth: "110px",
        cell : row =>(
            <div>
            {
                row[games_id_key["double pana"]]
            }
            </div>
        )
      },
      {
        name: "Tripple Pana",
        selector: "Tripple Pana",
        sortable: false,
        minWidth: "110px",
        cell : row =>(
            <div>
            {
                row[games_id_key["tripple pana"]]
            }
            </div>
        )
      },
      {
        name: "Half Sangam",
        selector: "Half Sangam",
        sortable: false,
        minWidth: "110px",
        cell : row =>(
            <div>
            {
                row[games_id_key["half sangam"]]
            }
            </div>
        )
      },
      {
        name: "Full Sangam",
        selector: "Full Sangam",
        sortable: false,
        minWidth: "110px",
        cell : row =>(
            <div>
            {
                row[games_id_key["full sangam"]]
            }
            </div>
        )
      },
  ...
  componentDidMount(){
    this.props.get_bets_fromresultannouncer(this.props.parsedFilter,this.props.bazaritem,this.props.date);
  }
  ...
  Select_bazar = async row =>{
    ...
    if(res){
      console.log(row);
      console.log(this.props.bazaritem);
