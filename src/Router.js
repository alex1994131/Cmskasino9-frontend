...
import {firstpage_load} from "./redux/actions/CMS/firstpage/index"
  ...
  const BetSports = lazy(()=> import('./views/reports/Sports/Bets/index'));
  ...
  const CredentialSetting = lazy(()=> import('./views/setting/providercredential/menumanager'));
  ...
  const MatkaResult = lazy(()=> import('./views/matka/result'));
...
const RequireAuth = (data) => {
  ...
  if (data.children && sidebararray && sidebararray.length > 0)  {
    ...
    for (var i in items) {
      ...
      let routers = ["/","/chagepassword","/Players/infor","/finace/playershow"];// special router
...
class AppRouter extends React.Component {
  ...
  componentWillMount(){
    ...
    if (get_sess) {
      ...
      this.props.firstpage_load(); // logo loading
        ...
        <Switch>
          ...
          {/*auth router*/}
            <RequireAuth  sidebararray={this.props.sidebararray}>
              ...
              <AppRoute path="/matka/result" component={MatkaResult} />
